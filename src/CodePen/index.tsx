...
export interface ICodePenOption {
  title?: string;
  html?: string;
  js?: string;
  css?: string;
...
export interface ICodePen {
  ...
  options?: ICodePenOption;
  btnText?: string;
}

const defaultOptions: ICodePenOption = {
  title: '',
  html: '',
  js: '',
  css: '',
...
export default function CodePen(props: ICodePen = {}) {
  const { options, btnText = 'CodePen', prefixCls, ...other } = props;
  const CodePenOption: ICodePenOption = { editors: '0010', js_pre_processor: 'babel', ...defaultOptions, ...options };

  return (
    <form action="https://codepen.io/pen/define" method="POST" target="_blank" {...other} className={`${prefixCls}-form`}>
